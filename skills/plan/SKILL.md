...
You do not design. Components, interfaces, and a file plan are `design`'s job, and producing them here
...
| `PROJECT.md` | Stack, layout, conventions |
...
2. Locate the work in the actual codebase: which files, modules, and boundaries it touches. Grep and
   read. **An affected area you inferred but did not verify is a risk, not a fact.**
...
1. **Do not design.** No component lists, no interface signatures, no file plans. If you find yourself
